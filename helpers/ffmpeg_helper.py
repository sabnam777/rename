...
async def MergeVideo(input_file: str, user_id: int, message: Message, format_: str):
    ...
    """
    output_vid = f"downloads/{str(user_id)}/[@bae_wafaaa].{format_.lower()}"
...
async def MergeSub(filePath: str, subPath: str, user_id):
    ...
    maps += f"-map 1:s "
    metadata += f"-metadata:s:s:{subTrack} title='Track {subTrack+1} - tg@bae_wafaaa' "
    subTrack += 1
...
def MergeAudio(videoPath: str, files_list: list, user_id):
    ...
    for j in range(1, len(files_list)):
        maps += f"-map {j}:a "
    LOGGER.info(
        f"Command: ffmpeg -hide_banner {inputfiles}-map 0:v:0 -map 0:a:? {maps}-map 0:s:? -c:v copy -c:a copy -c:s copy {rmDispositions}-disposition:a:{audioTracks} default 'downloads/{str(user_id)}/[@bae_wafaaa]_export.mkv'"
    )
